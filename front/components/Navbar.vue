<template>
  <header
    :class="
      transparent == true
        ? 'header-2 ' + navbarClass
        : 'header-2 change-color ' + navbarClass
    "
  >
  ...
  computed: {
    numberItems() {
      return this.$store.getters["getCartAmount"] != 0
        ? this.$store.getters["getCartAmount"].amount
        : 0;
    }
  },
    ...
    async getLocalStorageOrders() {
      if (process.browser) {
        ...
        if (order != null) {
          ...
          await this.$store.dispatch("storeCartAmount", {
            amount: numberItems
          });
        }
      }
    ...
    async getConfig() {
      ...
      if (config.data.secondaryLogo) {
        if (process.browser) {
          localStorage.setItem(
            "secondaryLogo",
            process.env.SERVER_URL + config.data.secondaryLogo
          );
        }
      }
      ...
      if (config.data.footerLogo) {
        if (process.browser) {
          localStorage.setItem(
            "footerLogo",
            process.env.SERVER_URL + config.data.footerLogo
          );
        }
      }
